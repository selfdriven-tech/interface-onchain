...
const getOwnWithdrawalsTotal = (withdrawals, rewardAccounts) => util_1.BigIntMath.sum(withdrawals.filter(({ stakeAddress }) => rewardAccounts.includes(stakeAddress)).map(({ quantity }) => quantity));
const computeImplicitCoin = ({ stakeKeyDeposit, poolDeposit }, { certificates, proposalProcedures, withdrawals = [] }, rewardAccounts, dRepKeyHash) => {
    ...
    const allWithdrawalsTotal = util_1.BigIntMath.sum(withdrawals.map(({ quantity }) => quantity));
